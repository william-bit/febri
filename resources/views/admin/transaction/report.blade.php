    ...
    <div style="margin-top: 10px">
                ...
                @if ($table['data']->count())
                    @foreach ($table['data'] as $datum)
                        <tr class="border-b border-gray-200">
                            <td class="td-w">
                                {{ isset($no) ? ($no += 1) : ($no = 1 + $table['data']->perpage() * ($table['data']->currentpage() - 1)) }}
                            </td>
                            @foreach ($table['order'] as $name => $header)
                                @if (strpos($name, '.') !== false)
                                    ...
                                    @if (isset($table['concat']))
                                        @if (in_array($table['concat'], $name))
                                            <td class="td-s">
                                                {{ implode(',', array_column($datum->{$relationship}->toArray(), $column)) }}
                                            </td>
                                        @else
                                ...
                                @elseif (!empty($table['valueConvert'][$name]))
                                    <td class="td-s">
                                        {{ $table['valueConvert'][$name][array_search($datum->{$name}, array_column($table['valueConvert'][$name], 'id'))]['value'] }}
                                    </td>
                                @else
                                    @php
                                        $custom = false;
                                    ...
                                    @if (!empty($table['json']))
                                        @if (in_array($name, $table['json']))
                                            ...
                                            <td class="p-2">
                                                <table border="1"
                                                    class="w-full border-collapse border-black border-solid border-1">
                                                    <tr class="">
                                                        <td class="w-6">No</td>
                                                    ...
                                                    @foreach (json_decode($datum->{$name}, true) as $jsonKey => $jsonValue)
                                                        <tr class="">
                                                            ...
                                                            <td class="w-2/3">
                                                                {{ !empty($jsonValue['quantity']) ? $jsonValue['quantity'] : '' }}
                                                            </td>
                                                            <td class="pl-2">
                                                                Rp.{{ number_format($jsonValue['price']) }} </td>
                                                            <td class="pl-2">
                                                                Rp.{{ number_format($jsonValue['price'] * $jsonValue['quantity']) }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </table>
                                    ...
                                    @if (!empty($table['photo']))
                                        @if (in_array($name, $table['photo']))
                                            ...
                                            @endphp
                                            <td class="p-2">
                                                <a href="{{ asset('storage/images/' . $datum->{$name}) }}"
                                                    data-lightbox="{{ $datum->{$name} }}"
                                                    data-title="{{ $datum->{$name} }}">
                                                    <img class="w-24 h-24 rounded"
                                                        src="{{ asset('storage/images/' . $datum->{$name}) }}"
                                                        alt="" title="">
                                                </a>
                                            </td>
                                        @endif
        ...
        <div class="relative">
            <div class="mt-2" style="text-align: right">
                ...
                <div>
                    <img class="inline-block h-24" src="{{ asset('storage/asset/ttd.jpg') }}" alt=""
                        tie="">
                </div>
