        ...
        <div class="flex flex-col items-center justify-center flex-1 w-full mt-12">
            <div class="flex-col w-1/2 p-10 bg-white rounded-lg">
                ...
                <div class="flex flex-row">
                    ...
                    <span class="w-1/3 m-2 mt-6 text-xl text-blue-500 border-b-2">Month</span>
                </div>
                <div class="border-b-4 cart-items">
                    ...
                    @foreach ($products as $product)
                        ...
                        <div class="flex flex-row cart-row">
                            <span
                                class="flex flex-row items-center w-1/3 mx-2 mt-2 space-x-2 text-xl text-blue-900 border-b-2">
                                <a href="{{ asset('storage/images/' . $product->photo) }}"
                                    data-lightbox="{{ $product->name }}" data-title="{{ $product->name }}">
                                    <img class="w-24 h-24" src="{{ asset('storage/images/' . $product->photo) }}"
                                        alt="" title="">
                                </a>
                                <span class="max-w-0">{{ $product->name }}</span>
                            </span>
                            <span
                                class="flex flex-row items-center w-1/4 mx-2 mt-2 text-xl text-blue-900 border-b-2 cart-price">
                                Rp.{{ number_format($product->price, 2) }}
                            </span>
                            <div
                                class="flex items-center justify-end w-1/3 mx-2 mt-2 space-x-3 text-xl text-blue-900 border-b-2 cart-quantity">
                                <input type="hidden" name="item[id][]" value="{{ $product->id }}">
                                <div>
                                    <input type="number" name="item[quantity][]" value="1"
                                        class="w-16 p-1 bg-white border-4 border-gray-200 rounded-lg cart-quantity-input">
                                    <span>Month</span>
                                </div>
                                <button value="{{ $product->id }}"
                                    class="p-1.5 ml-9 font-bold text-white bg-red-600 rounded-lg hover:bg-red-700 remove-button">Remove</button>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="flex items-center justify-center flex-1 w-full mt-12">
                <div class="flex-col w-1/2 p-10 bg-white rounded-lg">
                    ...
                    <div class="mt-5">
                        <x-form-file value='' label='Picture Proof' placeholder='Bukti Pembayaran' name='photo'>
                        </x-form-file>
                    </div>
                    <div class="flex flex-col">
                        @if (auth()->user())
                            <button
                                class="px-3 py-2 mt-12 font-bold text-white bg-green-700 border-2 focus:outline-none rounded-xl hover:bg-green-800">Pay</button>
                        @else
                            <a href="{{ route('login') }}"
                                class="px-3 py-2 mt-12 font-bold text-center text-white bg-green-700 border-2 focus:outline-none rounded-xl hover:bg-green-800">Bayar</a>
                        @endif
                    </div>
                </div>
            </div>
